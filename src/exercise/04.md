...
When using a `useEffect`, if it's going to mutate the DOM straight after the
component updates, in a way that is gonna change the DOM appearance, then it's
better to use `useLayoutEffect`. This avoid a little "flicker" the user could
see, because `useLayoutEffect` runs before the browser repaint.

So use:

- `useEffect`: all the time
- `useLayoutEffect`: If you need to mutate the DOM in an observable way to the
  user and/or do need to perform measurements
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=04%3A%20useLayoutEffect%3A%20auto-scrolling%20textarea&em=yoh.legrand%40gmail.com).
