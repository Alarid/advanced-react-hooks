...
`React.useRef` is pretty versatile. First argument is the reducer itself (taking
current state and new state as arguments), second is the initial state. It can
take a third argument for lazy initialization, which is a function receiving the
initial state provided as the second argument.

`const [state, setState] = React.useRef(myReducer, initialState, lazyInitFn)`

`const myReducer = (state, action) => ({...state, ...action})`
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=01%3A%20useReducer%3A%20simple%20Counter&em=yoh.legrand%40gmail.com).
