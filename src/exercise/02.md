...
Passing a function defined inside a component as dependency for a `useEffect`
"doesn't work", because this function will change at every render (being
redefined), and so the hook is called every render, which is probably not what
you want originally. This is the problem `useCallback` solves, because it
returns a memoized version of the function so we can pass it as a dependency to
the `useEffect`. This memoized version will only change if its own dependencies
have changed, which will trigger a call to the `useEffect` as well.

`useCallback` (and `useMemo`) provide value stability, helping us making sure we
control when our side effects run. When its dependency array is empty, it will
never change.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=02%3A%20useCallback%3A%20custom%20hooks&em=yoh.legrand%40gmail.com).
