...
Prop drilling / threading is the process we have to go through when getting data
down in the component tree to various components

Good practice: when creating a context, it's good practice to wrap to provider
in custom component, so it can manage its own state, accept props, etc. Also, to
prevent component outside of the provider from using the context, it's a good
practice to associate a custom hook with the context, that will use it, throw an
error if it is undefined (used ouside of the provider), and return the context
value
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=03%3A%20useContext%3A%20simple%20Counter&em=yoh.legrand%40gmail.com).
